...
const AddCustomerOrderForm = () => {
			...
			const resultArray = Object.keys(groupedData)
				.map((productId) => {
					...
					const branches = productData
						...
						.filter(Boolean);
					const totalQuantity = branches.reduce(
						(total: number, branch: any) => total + branch.quantity,
						0,
					);
					return {
						productId,
	...
	const handleProductChange = (selectedOption: any, index: any) => {
		const selectedProduct = productsArray.find(
			(product: any) => product.productId === selectedOption.value,
		);
		if (selectedProduct) {
			const {
				productId,
	...
	useEffect(() => {
		...
		for (const entry of entries) {
			const product = productsData?.find(
				(prod: any) => prod._id.toString() === entry.product.toString(),
			);
	...
	useEffect(() => {
		...
		for (const entry of entries) {
			const product = productsData?.find(
				(product: any) => product._id.toString() === entry.product.toString(),
			);
			totalCoatingCharges +=
				entry?.quantity * entry?.coating_rate * (entry?.length || product?.length || 0);
		}
		if (discount) {
	...
	return (
		<PageWrapper name='ADD PRODUCTS' isProtectedRoute={true}>
          ...
          <div className='m-5 mb-4 grid w-full grid-cols-6 gap-1'>
								...
								{entries &&
									entries.map((entry: any, index: any) => (
										<>
											...
											<div
												key={index}
												className='mt-2 grid grid-cols-12 gap-3'>
												{!productTransfer ? (
													<>
												...
												<div className='col-span-12 lg:col-span-2'>
													...
													<CreateSelectReact
														...
														value={
															entry.coating || selectedCoatings[index]
																? {
																		value: entry.coating,
																		label: coatingData?.find(
																			(item: any) =>
																				item?._id ===
																				entry.coating,
																		)?.name,
												...
												{entry.coating && (
													<div className='col-span-12 lg:col-span-2'>
														...
														<Select
															...
															name={`color-${index}`}
															value={
																entry.color || selectedColor[index]
															}
															onChange={(e) =>
												...
												<div className='col-span-12 lg:col-span-2'>
													...
													<Input
														...
														onChange={(e) => {
															const updatedEntries = [...entries];
															updatedEntries[index].coating_rate =
																e.target.value;
															setEntries(updatedEntries);
														}}
													/>
												...
												<div className='col-span-12 lg:col-span-2'>
													...
													<Checkbox
														...
														onChange={(e) => {
															...
															const updatedEntries = [...entries];
															updatedEntries[index].withoutMaterial =
																target.checked; // Update with the checked value
															setEntries(updatedEntries);
														}}
													/>
												</div>
												{coatingData.find(
													(coating: any) => coating._id === entry.coating,
												)?.type === 'anodize' && (
													<div className='col-span-12 lg:col-span-2'>
														...
														<Select
															...
															<option value='12 Micron'>
																12 Micron
															</option>
															<option value='15 Micron'>
																15 Micron
															</option>
															<option value='20 Micron'>
																20 Micron
															</option>
														</Select>
													</div>
			...
			<div className='col-span-12 mt-5 flex flex-col gap-1 xl:col-span-6'>
				<Card>
					<CardBody>
						<div className='flex'>
							<div className='bold w-full'>
