...
const InHouseComponentListPage = () => {
	const [inHouseComponentData, setInHouseComponentData] = useState<any>([]);
	const [isLoading, setIsLoading] = useState<boolean>(false);
	const [globalFilter, setGlobalFilter] = useState<string>('');
	const [sorting, setSorting] = useState<SortingState>([]);
	const [deleteModal, setDeleteModal] = useState<boolean>(false);
	const [inHouseComponentId, setInHouseComponentId] = useState<string>('');
	...
	const columns: any[] = [
		...
		columnHelper.accessor('inHouseComponentType', {
			cell: (info: any) => (
				<Link to={`${listLinkPath}${info.row.original.id}`}>
					<div className=''>{info.row.original.inHouseComponentType || 'NA'}</div>
				</Link>
			),
	...
	const handleClickDelete = (info: CellContext<ERPVendor, unknown>) => {
		...
		setInHouseComponentId(info.row.original.id ?? '');
	};
	...
	return privileges.canRead() ? (
		<PageWrapper name='In House Component List'>
			<Subheader>
				<SubheaderLeft>
					<FieldWrap
			...
			<Container>
				<Card className='h-full'>
					<CardHeader>
						<CardHeaderChild>
							<CardTitle>In House Components</CardTitle>
							<Badge
								variant='outline'
...
export default InHouseComponentListPage;
