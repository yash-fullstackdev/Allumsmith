...
const EditWorkerModal = ({ workerId, setIsEditModal, fetchData }: any) => {
  ...
  const fetchWorkerById = async () => {
    try {
      const worker = await get(`/workers/${workerId}`);
  ...
  const editWorker = async () => {
    ...
    try {
      const editedWorker = await put(`/workers/${workerId}`, formData);
