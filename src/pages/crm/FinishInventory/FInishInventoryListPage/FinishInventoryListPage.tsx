	...
	const columns = [
		columnHelper.accessor('name', {
			cell: (info) => {
				return (
					<div
						className='flex  min-h-[30px]  items-center text-xl'
						onClick={info?.row.getToggleExpandedHandler()}
						style={{ cursor: 'pointer' }}>
						{info?.row.getCanExpand() ? (
							<Button
								rightIcon={info?.row.getIsExpanded() ? 'HeroMinus' : 'HeroPlus'}
							/>
						) : null}
						{`${info.getValue()}`}
					</div>
			...
			header: () => (
				<div className='flex min-h-[30px] items-center text-xl'>Product Name</div>
			),
		}),
		columnHelper.accessor('totalQty', {
			cell: (info) => (
				<div className='flex  min-h-[30px]  items-center text-xl'>
					{`${info.getValue()}`}
				</div>
			),
			header: () => (
				<div className='flex min-h-[30px] items-center text-xl'>Total Quantity</div>
			),
		}),
	];
