...
const AddInvoice = () => {
	...
	const getPurchaseOrderByid = async () => {
		try {
			...
			allPurchaseOrderById?.entries?.forEach((entries: any) => {
				...
				productCoatingRate?.push(
					entries?.product?.length *
						entries?.coating_rate *
						entries?.itemSummary?.coatingQuantity,
				);
			});
	...
	const calculateTotals = (updatedTotalWeights: number[], updatedTotalCoatingRate: number[]) => {
		...
		const totalCoatingPrice =
			updatedTotalCoatingRate.reduce((acc, item) => acc + item, 0) * (1 - discount / 100);
		...
		const grandTotal = parseFloat(
			(estimateGrandTotal + amountAfterGst + amountAfterTax).toFixed(2),
		);
	...
	const handleDeliveredQuantityChange = (value: any, index: number) => {
		...
		const totalWeight =
			purchaseOrderData.entries[index]?.product?.weight * parseFloat(value) || 0;
		const updatedTotalWeights = [...totalWeights];
		updatedTotalWeights[index] = parseFloat(totalWeight.toFixed(2));
		setTotalWeights(updatedTotalWeights);
		const totalRate = purchaseOrderData.entries[index]?.coating?.coating_rate
			? purchaseOrderData.entries[index]?.product?.length *
			  parseFloat(value) *
			  purchaseOrderData.entries[index]?.coating?.coating_rate
			: purchaseOrderData.entries[index]?.product?.length *
			  parseFloat(value) *
			  purchaseOrderData.entries[index]?.coating_rate;
		const updatedTotalCoatingRate = [...totalCoatingRate];
	...
	const calcTotalWeight = () => {
		let weight = 0;
		totalWeights.forEach((data: any) => (weight += data));
	...
	const handleSaveEntries = async () => {
		...
		try {
			...
			const totalSpecificProductPrice = purchaseOrderData.entries.reduce(
				(acc: number, entry: any, index: number) => {
					const specificProductPrice =
						entry.product?.weight *
						parseInt(entry.product?.rate) *
						deliveredQuantities[index];
			...
			const totalCoatingTotal = purchaseOrderData.entries.reduce(
				(acc: number, entry: any, index: number) => {
					const coatingTotal =
						(parseFloat(entry?.coating?.coatingWeight) || 0) *
						parseFloat(entry?.coating?.rate);
					const finalCoatingTotal =
						coatingTotal -
						(coatingTotal *
							(parseFloat(quantityAndDiscounts[index]?.coating_discount) || 0)) /
							100;
			...
			const payload = {
				...
				products: purchaseOrderData?.entries.map((entry: any, index: any) => {
					const specificProductPrice =
						...
						deliveredQuantities?.[index];
					const coatingTotal =
						(entry?.coating?.coatingWeight || 0) * entry?.coating?.rate;
					const finalCoatingTotal =
						coatingTotal -
						(coatingTotal *
							(parseFloat(quantityAndDiscounts?.[index]?.coating_discount) || 0)) /
										...
										<div>
											<div className='mt-2 grid grid-cols-12 gap-1'>
												...
												<div className='col-span-12 lg:col-span-12'>
													{customerId &&
														Array.isArray(purchaseOrderData.entries) &&
														purchaseOrderData.entries.map(
															(entry: any, index: number) => {
																return (
																	<>
																		...
																		<div
																			className='mt-2 grid grid-cols-12 gap-1'
																			key={entry._id}>
