...
import { ErrorMessage, Input, Label, Select, SelectReact } from '../../../../components/form';
...
const AddPurchaseOrderForm = () => {
	...
	const handleSubmit = async (value: any) => {
		try {
			...
			const self_products = value?.self_products
				?.map((item: any) => {
					if (item?.product?._id) {
						const obj = {
																		...
																		(
																			...
																			return (
																				<div
																					...
																					className='col-span-12 mt-[10px] grid grid-cols-12 gap-1 lg:col-span-12'>
						...
						<div>
							<div>
								{formik?.values?.self_products?.length
									? formik?.values?.self_products?.map(
											(entry: any, index: any) => {
												return (
													<div>
														<div className='mt-2 flex items-end justify-end'>
