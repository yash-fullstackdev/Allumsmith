				...
				return (
					<div className=''>
						<Select
							...
							disabled={
								!isNewPurchaseEntry || info.row.original.status === 'completed'
							}>
							{branchesData &&
	...
	const handleSave = async () => {
		const saveData = table.getFilteredRowModel().rows.map((row: any, index: number) => ({
			...
			receivedQuantity: parseFloat(
				editedData[row.id]?.receivedQuantity ?? row.original.receivedQuantity,
			),
			requiredQuantity: parseFloat(row.original.requiredQuantity),
			branch: selectedBranches[index],
		}));
		const UpdatedEntries: any = saveData.filter(
			(entry) =>
				entry.ProductStaus !== 'completed' && entry?.branch && entry.receivedQuantity,
		);
		try {
			const invalidEntries = purchaseOrderData?.products?.some(
				(entry: any) =>
					entry.requiredQuantity - entry.receivedQuantity <
					UpdatedEntries?.receivedQuantity,
			);
			if (invalidEntries) {
				toast.error(
					'Received quantity cannot be greater than required quantity for some products',
				);
				return;
			}
			...
			const savePurchaseEntry = await post(
				`/purchase-order/registerPurchaseEntry/${poId}`,
				finalUpdatedvalues,
			);
